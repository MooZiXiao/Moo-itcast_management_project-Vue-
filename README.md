...
**views文件夹目录中创建goods文件夹，goods中创建组件（goods.vue，list.vue，add.vue）**

**配置路由，在goods嵌套路由（加入`<roter-view/>`）**
...
**api文件夹目录中创建goodsIndex.js，创建获得商品数据接口，通过mounted调用显示数据****

**搜索通过`v-model='goodsParams.query'`及调用封装的`init`实现查询**
